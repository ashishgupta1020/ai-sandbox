...
(function () {
  ...
  const gridUtils = Taskman.grid || {};
  const api = Taskman.api || {};

  const getQueryParam = utils.getQueryParam;
  const getRemarksHTML = utils.getRemarksHTML;
  const buildFieldEditor = utils.buildFieldEditor;
  const gridAvailable = gridUtils.available;
  const renderGrid = gridUtils.renderGrid;
  const focusGridRow = gridUtils.focusGridRow;
  let GRID = null;
  ...
  const rawFocusTaskId = getQueryParam ? getQueryParam('taskId') : null;
  ...
  function editableFormatter(field, type, options) {
    return (cell, row) => {
      ...
      if (type === 'markdown') {
        ...
        const displayHTML = (isEmpty || typeof getRemarksHTML !== 'function') ? null : getRemarksHTML(cell);
  ...
  function actionsFormatter() {
    return (_, row) => {
      ...
      const toggleHighlight = async () => {
        try {
          const resp = await api.highlightTask(CURRENT_PROJECT, tid, !isHighlighted);
      ...
      const deleteTask = async () => {
        ...
        if (!confirmed) return;
        try {
          const resp = await api.deleteTask(CURRENT_PROJECT, tid);
  ...
  function focusTaskRow(taskId) {
    ...
    if (!box || typeof focusGridRow !== 'function') { pendingFocusTaskId = null; return; }
    focusGridRow(box, GRID, idx0, { onDone: () => { pendingFocusTaskId = null; } });
  }
  ...
  function startInlineEditor(target, index0, field, type, options, currentValue, taskId) {
    ...
    const commit = async (value) => {
      try {
        const resp = await api.updateTask(CURRENT_PROJECT, taskId, { [field]: value });
        if (resp && resp.ok && typeof resp.id === 'number') {
          const idx = findRowIndexById(resp.id);
  ...
  function renderWithGrid(rows) {
    ...
    GRID_COLUMNS = [
      { id: 'idx0', name: '_idx0', hidden: true }, // hidden stable 0-based index
      { id: 'id', name: '_id', hidden: true },   // hidden task id (server truth)
      { id: 'index', name: 'Index', sort: true },
      { id: 'summary', name: 'Summary', sort: true, formatter: editableFormatter('summary',  'text') },
      { id: 'assignee', name: 'Assignee', sort: true, formatter: editableFormatter('assignee', 'text') },
      { id: 'status', name: 'Status', sort: true, formatter: editableFormatter('status',   'select', STATUS_OPTS) },
      { id: 'priority', name: 'Priority', sort: true, formatter: editableFormatter('priority', 'select', PRIORITY_OPTS) },
      { id: 'remarks', name: 'Remarks', sort: false, width: '30%', formatter: editableFormatter('remarks', 'markdown') },
      { id: 'actions', name: '', sort: false, formatter: actionsFormatter() }
    ];
  }
  ...
  if (typeof renderGrid === 'function') {
    GRID = renderGrid(box, GRID, gridConfig, { preserveHeight: true });
    return;
  }

  box.textContent = 'Grid support is unavailable. Refresh or try again later.';
  }
  ...
  function renderFallbackTable(tasks) {
    ...
    tasks.forEach((t, i) => {
      ...
      const src = t.remarks || '';
      const html = typeof getRemarksHTML === 'function' ? getRemarksHTML(src) : null;
      if (html) {
  ...
  async function loadTasks(name) {
    const box = document.getElementById('tasks');
    try {
      const data = await api.listProjectTasks(name);
      const tasks = Array.isArray(data.tasks) ? data.tasks : [];
      if (typeof gridAvailable === 'function' && gridAvailable()) {
  ...
  function wireTasksUI() {
    ...
    btnAddTask.addEventListener('click', () => {
      ...
      btnSave.addEventListener('click', async () => {
        ...
        try {
          const resp = await api.createTask(CURRENT_PROJECT, payload);
